<script lang="ts">
  ...
  import { type ArkGridGem, type ArkGridGemOptionName } from '../lib/models/arkGridGems';
  import {
    Core,
    Gem,
    GemSet,
  ...
  function runSolve(isSupporter: boolean) {
    ...
    for (const { attr, gsp } of [
      ...
      if (needLauncherGem[attr] && gsp) {
        ...
        console.log(attr, '추가 젬 계산', performance.now() - start, 'ms');
      }
    }
